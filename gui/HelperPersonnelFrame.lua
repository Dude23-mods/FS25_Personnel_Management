...
HelperPersonnelFrame.MODE_OVERVIEW = 1
...
HelperPersonnelFrame.WORKER_DETAIL_ROW_HEIGHT = 0.128
HelperPersonnelFrame.WORKER_DETAIL_ROW_STEP = 0.132
HelperPersonnelFrame.DETAIL_TEXT_SIZE = 0.0106
HelperPersonnelFrame.DETAIL_TEXT_LINE_STEP = 0.014
HelperPersonnelFrame.OVERVIEW_HISTORY_VISIBLE_ROWS = 8
...
function HelperPersonnelFrame.new(subclass_mt, messageCenter)
    local self = TabbedMenuFrameElement.new(nil, subclass_mt or HelperPersonnelFrame_mt)
    ...
    self.btnOpenApplicants = hpApplyPausedButton({
        inputAction = hpGetInputAction("HP_OPEN_APPLICANTS", nil),
...
function HelperPersonnelFrame:setContext(app)
    ...
    if self.app ~= nil and self.app.modDir ~= nil then
        ...
        self.iconOverlay = Overlay.new(Utils.getFilename("gui/menuicon_personell.dds", self.app.modDir), 0, 0, 0.03, 0.055)
        self:loadPortraitOverlays()
    end
end

function HelperPersonnelFrame:delete()
    if self.lineOverlay ~= nil then
        self.lineOverlay:delete()
        self.lineOverlay = nil
    end
    ...
    HelperPersonnelFrame:superClass().delete(self)
end

function HelperPersonnelFrame:onFrameOpen()
    HelperPersonnelFrame:superClass().onFrameOpen(self)
...
function HelperPersonnelFrame:copyAttributes(src)
    HelperPersonnelFrame:superClass().copyAttributes(self, src)
end

function HelperPersonnelFrame:initialize() end
function HelperPersonnelFrame:onGuiSetupFinished() end
...
function HelperPersonnelFrame:onInputConfirm(_, inputValue)
    ...
    if self.mode == HelperPersonnelFrame.MODE_OVERVIEW then
...
function HelperPersonnelFrame:onInputCancel(_, inputValue)
    if inputValue ~= 1 then
        return
    end
...
function HelperPersonnelFrame:onClickActivate()
    if self.mode == HelperPersonnelFrame.MODE_OVERVIEW then
        self:showWorkersView()
    else
        self:activateCurrentEntry()
...
function HelperPersonnelFrame:onClickExtra1()
    if self.mode == HelperPersonnelFrame.MODE_OVERVIEW then
...
function HelperPersonnelFrame:onClickBack()
    if self.mode ~= HelperPersonnelFrame.MODE_OVERVIEW then
        self:showOverview()
        return true
    end
...
function HelperPersonnelFrame:onClickPagePrevious()
    local inGameMenu = self:getInGameMenu()
...
function HelperPersonnelFrame:onClickPageNext()
    local inGameMenu = self:getInGameMenu()
...
function HelperPersonnelFrame:showOverview()
    self.mode = HelperPersonnelFrame.MODE_OVERVIEW
    self:updateButtons()
end

function HelperPersonnelFrame:showWorkersView()
    local workers = self:getWorkers()
    if #workers == 0 then
        return
    end

    self.mode = HelperPersonnelFrame.MODE_WORKERS
    self.workerIndex = hpClamp(self.workerIndex, 1, #workers)
    self:updateButtons()
end

function HelperPersonnelFrame:showApplicantsView()
    local applicants = self:getApplicants()
    if #applicants == 0 then
        return
    end

    self.mode = HelperPersonnelFrame.MODE_APPLICANTS
...
function HelperPersonnelFrame:refresh()
    ...
    if self.mode == HelperPersonnelFrame.MODE_WORKERS and #workers == 0 then
        self.mode = HelperPersonnelFrame.MODE_OVERVIEW
    elseif self.mode == HelperPersonnelFrame.MODE_APPLICANTS and #applicants == 0 then
        self.mode = HelperPersonnelFrame.MODE_OVERVIEW
    end
...
function HelperPersonnelFrame:updateButtons()
    local buttons = {}
    ...
    self.btnOpenApplicants.inputAction = hpGetInputAction("HP_OPEN_APPLICANTS", nil)
    ...
    self.btnExecute.text = self.mode == HelperPersonnelFrame.MODE_APPLICANTS and self:getText("ui_button_hire", "Einstellen") or self:getText("ui_button_fire", "Entlassen")
...
function HelperPersonnelFrame:getMenuButtonInfo()
    return self.menuButtonInfo or {}
end

function HelperPersonnelFrame:getText(key, fallback)
    if g_i18n ~= nil then
...
function HelperPersonnelFrame:formatText(key, fallback, ...)
...
function HelperPersonnelFrame:getManager()
    return self.app ~= nil and self.app.manager or nil
end

function HelperPersonnelFrame:getWorkers()
    local manager = self:getManager()
    if manager == nil then
        return {}
    end
...
function HelperPersonnelFrame:getApplicants()
    local manager = self:getManager()
    if manager == nil then
        return {}
    end
...
function HelperPersonnelFrame:isWorkerActive(worker)
...
function HelperPersonnelFrame:getActiveWorkerEntries(workers)
    local activeWorkers = {}
...
function HelperPersonnelFrame:shortenText(text, maxLength)
...
function HelperPersonnelFrame:stripRuntimeHelperSuffix(value)
    if value == nil then
        return nil
    end
...
function HelperPersonnelFrame:normalizeDisplayNamePart(value)
    if value == nil then
        return nil
    end
...
function HelperPersonnelFrame:formatNameSegment(value)
...
function HelperPersonnelFrame:getValueFromObject(object, getterNames, directKeys)
...
function HelperPersonnelFrame:getBrandNameFromValue(value)
    if value == nil then
        return nil
    end
...
function HelperPersonnelFrame:getBrandNameFromConfigFileName(configFileName)
...
function HelperPersonnelFrame:getModelNameFromConfigFileName(configFileName)
    local filename = self:normalizeDisplayNamePart(configFileName)
    if filename == nil then
        return nil
    end
...
function HelperPersonnelFrame:getBrandNameFromObject(object)
    if type(object) ~= "table" then
        return nil
    end
...
function HelperPersonnelFrame:getModelNameFromObject(object)
    if type(object) ~= "table" then
        return nil
    end
...
function HelperPersonnelFrame:combineBrandAndModelName(brandName, modelName, fallback)
...
function HelperPersonnelFrame:getObjectDisplayName(object, fallback)
    fallback = fallback or ""
...
function HelperPersonnelFrame:getJobForWorker(worker)
...
function HelperPersonnelFrame:getRootVehicle(vehicle)
    if vehicle == nil then
        return nil
    end
...
function HelperPersonnelFrame:getVehicleDisplayName(vehicle, fallback)
...
function HelperPersonnelFrame:collectAttachedObjects(vehicle, result, visited, depth)
    result = result or {}
    visited = visited or {}
    depth = depth or 0
...
function HelperPersonnelFrame:getAttachedImplementText(vehicle)
...
function HelperPersonnelFrame:objectHasSpecialization(object, specializationName)
...
function HelperPersonnelFrame:getJobClassName(job)
    if job == nil then
        return ""
    end
...
function HelperPersonnelFrame:getLocalizedActivityText(key, fallback)
    return self:getText(key, fallback)
end

function HelperPersonnelFrame:detectActivityText(job, vehicle)
...
function HelperPersonnelFrame:getNumericFieldIdFromValue(value)
    if value == nil then
        return nil
    end
...
function HelperPersonnelFrame:getValueFromParameter(parameter)
...
function HelperPersonnelFrame:getFieldIdFromJob(job)
    if job == nil then
        return nil
    end
...
function HelperPersonnelFrame:getFieldIdFromFieldObject(field)
    if field == nil then
        return nil
    end
...
function HelperPersonnelFrame:getFrameTimeMs()
...
function HelperPersonnelFrame:debugFieldDetection(worker, message)
    if HelperPersonnelFrame.FIELD_DETECTION_DEBUG_LOGGING ~= true then
        return
    end
...
function HelperPersonnelFrame:addPositionSample(samples, x, z, source)
    x = tonumber(x)
    z = tonumber(z)
...
function HelperPersonnelFrame:addObjectPositionSamples(object, samples, sourcePrefix)
...
function HelperPersonnelFrame:addJobPositionSamples(job, samples)
...
function HelperPersonnelFrame:collectWorkPositionSamples(job, vehicle)
    local samples = {}
...
function HelperPersonnelFrame:getVehicleWorldPosition(vehicle)
    local rootVehicle = self:getRootVehicle(vehicle)
    local samples = {}
...
function HelperPersonnelFrame:getFarmlandIdAtPosition(x, z)
...
function HelperPersonnelFrame:getFieldById(fieldId)
...
function HelperPersonnelFrame:getFieldFarmlandId(field)
...
function HelperPersonnelFrame:getFieldsForFarmlandId(farmlandId)
    local fields = {}
    local seen = {}
...
function HelperPersonnelFrame:getFieldCheckObjects(field)
    local objects = {field}
    if type(field) == "table" then
...
function HelperPersonnelFrame:getFieldContainsPosition(field, x, z)
...
function HelperPersonnelFrame:getFieldGroundAtPosition(x, z)
...
function HelperPersonnelFrame:getFieldIdAtPositionDetailed(x, z)
...
function HelperPersonnelFrame:getFieldIdAtPosition(x, z)
    local fieldId = self:getFieldIdAtPositionDetailed(x, z)
    return fieldId
end

function HelperPersonnelFrame:getFieldIdFromWorkPositions(worker, job, vehicle)
...
function HelperPersonnelFrame:getCachedFieldIdForActiveWorker(worker, job, vehicle)
    ...
    self.fieldDetectionCache[cacheKey] = {
        fieldId = fieldId,
        expiresAt = now + (HelperPersonnelFrame.FIELD_DETECTION_CACHE_MS or 3000)
    }
...
function HelperPersonnelFrame:keyEvent(unicode, sym, modifier, isDown)
    ...
    elseif sym == hpGetInputConstant("KEY_pageup", nil) or sym == hpGetInputConstant("KEY_pageUp", nil) then
        if self:moveDetailSelection(-(HelperPersonnelFrame.DETAIL_VISIBLE_COUNT or 4)) then
            return true
        end
...
function HelperPersonnelFrame:resetClickAreas()
    self.clickAreas = {}
    self.detailListMouseArea = nil
    self.detailScrollbarMouseArea = nil
    self.overviewHistoryMouseArea = nil
...
function HelperPersonnelFrame:addClickArea(x, y, width, height, action, index)
...
function HelperPersonnelFrame:isPointInClickArea(posX, posY, area)
    return area ~= nil and posX >= area.x and posX <= area.x + area.width and posY >= area.y and posY <= area.y + area.height
end

function HelperPersonnelFrame:isOverviewMode()
    return self.mode == HelperPersonnelFrame.MODE_OVERVIEW
end

function HelperPersonnelFrame:getOverviewHistoryScrollState()
    ...
    local count = #rows
    local visibleCount = math.min(count, HelperPersonnelFrame.OVERVIEW_HISTORY_VISIBLE_ROWS or 8)
...
function HelperPersonnelFrame:setOverviewHistoryFirstRow(firstRow)
...
function HelperPersonnelFrame:scrollOverviewHistory(deltaRows)
    local state = self:getOverviewHistoryScrollState()
    if state == nil then
        return false
    end
...
function HelperPersonnelFrame:setOverviewHistoryScrollFromMouseY(posY, area)
    local state = self:getOverviewHistoryScrollState()
...
function HelperPersonnelFrame:isDetailMode()
    return self.mode == HelperPersonnelFrame.MODE_WORKERS or self.mode == HelperPersonnelFrame.MODE_APPLICANTS
end

function HelperPersonnelFrame:getDetailScrollState()
    if not self:isDetailMode() then
        return nil
    end

    local isApplicantView = self.mode == HelperPersonnelFrame.MODE_APPLICANTS
    local entries = isApplicantView and self:getApplicants() or self:getWorkers()
    local count = #entries
    local visibleCount = math.min(count, HelperPersonnelFrame.DETAIL_VISIBLE_COUNT or 4)
...
function HelperPersonnelFrame:setDetailSelection(selectedIndex, firstIndex)
...
function HelperPersonnelFrame:moveDetailSelection(delta)
    local state = self:getDetailScrollState()
    if state == nil then
        return false
    end
...
function HelperPersonnelFrame:scrollDetailList(deltaRows)
    local state = self:getDetailScrollState()
    if state == nil then
        return false
    end
...
function HelperPersonnelFrame:setDetailScrollFromMouseY(posY, area)
    local state = self:getDetailScrollState()
    if state == nil or area == nil or area.height <= 0 then
        return false
    end
...
function HelperPersonnelFrame:isMouseWheelUp(button)
...
function HelperPersonnelFrame:isMouseWheelDown(button)
...
function HelperPersonnelFrame:handleOverviewHistoryMouseScroll(posX, posY, button)
    if not self:isOverviewMode() then
        return false
    end
...
function HelperPersonnelFrame:handleOverviewHistoryScrollbarDrag(posX, posY, isDown, isUp)
    if not self:isOverviewMode() then
        self.overviewHistoryScrollbarDragging = false
        return false
    end
...
function HelperPersonnelFrame:handleDetailMouseScroll(posX, posY, button)
    if not self:isDetailMode() then
        return false
    end
...
function HelperPersonnelFrame:handleDetailScrollbarDrag(posX, posY, isDown, isUp)
    if not self:isDetailMode() then
        self.detailScrollbarDragging = false
        return false
    end
...
function HelperPersonnelFrame:handleMouseClick(posX, posY)
...
function HelperPersonnelFrame:mouseEvent(posX, posY, isDown, isUp, button, eventUsed)
    local used = eventUsed == true
    local superFunc = HelperPersonnelFrame:superClass().mouseEvent
...
function HelperPersonnelFrame:draw()
    HelperPersonnelFrame:superClass().draw(self)
...
function HelperPersonnelFrame:drawHeader()
    if self.iconOverlay ~= nil then
        self.iconOverlay:setPosition(0.175, 0.84)
        self.iconOverlay:setDimension(0.032, 0.056)
...
function HelperPersonnelFrame:drawOverview()
    local manager = self:getManager()
    if manager == nil then
        return
    end
    ...
    local monthlyLines = self:getMonthlyReportLines(counts, applicants, workers)
    ...
    self:drawSeparator(0.22, monthlySeparatorY, 0.58)
    self:drawTextLine(0.51, monthlySeparatorY - 0.035, 0.015, RenderText.ALIGN_CENTER, self:getText("ui_monthlyReportHeader", "MONATSBERICHT"), 1, 1, 1, 1, true)

    local y = monthlySeparatorY - 0.066
    for _, line in ipairs(monthlyLines) do
        self:drawTextLine(0.24, y, 0.013, RenderText.ALIGN_LEFT, line, 0.61, 0.73, 0.07, 1, true)
        y = y - 0.026
    end
end

function HelperPersonnelFrame:drawDetail()
    local manager = self:getManager()
    if manager == nil then
        return
    end
    ...
    if isApplicantView then
        ...
        self:drawTextLine(0.22, 0.722, 0.0125, RenderText.ALIGN_LEFT, self:getText("ui_instructionWorkerTransport", "Mit T teilst du den Mitarbeiter für Transportaufgaben ein, mit S schulst du ihn."), 0.61, 0.73, 0.07, 1, true)
        self:drawTextLine(0.22, 0.702, 0.0125, RenderText.ALIGN_LEFT, self:getText("ui_instructionWorkerSalaryRaise", "Bei Gehaltsforderungen: Mit G gewährst du die Gehaltserhöhung, mit N lehnst du sie ab."), 0.61, 0.73, 0.07, 1, true)
        detailTitleY = 0.674
    ...
    if #entries == 0 then
        ...
        local y = listStartY
        local rowHeight = isApplicantView and (HelperPersonnelFrame.DETAIL_ROW_HEIGHT or 0.092) or (HelperPersonnelFrame.WORKER_DETAIL_ROW_HEIGHT or HelperPersonnelFrame.DETAIL_ROW_HEIGHT or 0.092)
        local rowStep = isApplicantView and (HelperPersonnelFrame.DETAIL_ROW_STEP or 0.097) or (HelperPersonnelFrame.WORKER_DETAIL_ROW_STEP or HelperPersonnelFrame.DETAIL_ROW_STEP or 0.097)
...
function HelperPersonnelFrame:getWorkerSummaryCounts(workers)
    local manager = self:getManager()
    local counts = {
        total = 0,
        available = 0,
...
function HelperPersonnelFrame:getMonthlyReportLines(counts, applicants, workers)
    local lines = {}

    if type(counts) ~= "table" then
        counts = self:getWorkerSummaryCounts(workers or {})
    end

    table.insert(lines, self:formatText("ui_monthlyLineStaff", "Personalbestand: Helfer %d | Fachkräfte %d | Profis %d", counts.helpers, counts.specialists, counts.experts))

    if #applicants > 0 then
        table.insert(lines, self:formatText("ui_monthlyLineApplicantsReady", "Bewerbermarkt bereit: %d Bewerber stehen zur Verfügung.", #applicants))
    else
        table.insert(lines, self:getText("ui_monthlyLineApplicantsEmpty", "Bewerbermarkt aktuell leer."))
    end

    if #workers == 0 then
        table.insert(lines, self:getText("ui_monthlyLineNoWorkers", "Noch keine Mitarbeiter eingestellt."))
    else
        table.insert(lines, self:formatText("ui_monthlyLineAvailableWorkers", "Verfügbare Mitarbeiter: %d von %d", counts.available, counts.total))

        local manager = self:getManager()
        if manager ~= nil and manager.getTotalWorkStats ~= nil then
            local jobs, minutes, monthlyWages = manager:getTotalWorkStats()
            local monthlyWagesText = g_i18n:formatMoney(monthlyWages or 0, 0, true, false)
            table.insert(lines, self:formatText("ui_monthlyLineWorkStats", "Bisher geleistet: %d Einsätze | %s | Monatsgehälter: %s", jobs or 0, manager:formatWorkMinutes(minutes or 0), monthlyWagesText))

            if manager.getLastPayrollText ~= nil then
                table.insert(lines, self:formatText("ui_monthlyLinePayroll", "Letzte Gehaltsabrechnung: %s", manager:getLastPayrollText()))
            end
        end
    end
...
function HelperPersonnelFrame:deletePortraitOverlays()
...
function HelperPersonnelFrame:getPortraitIndex(person)
    local count = math.max(1, self.portraitCount or #HelperPersonnelFrame.PORTRAIT_FILENAMES or 1)
...
function HelperPersonnelFrame:getFallbackPortraitFilename(person)
    return HelperPersonnelFrame.PORTRAIT_FILENAMES[self:getPortraitIndex(person)] or HelperPersonnelFrame.PORTRAIT_FILENAMES[1]
end

function HelperPersonnelFrame:getPortraitFilename(person)
...
function HelperPersonnelFrame:createPortraitOverlay(filename)
...
function HelperPersonnelFrame:getPortraitOverlay(person)
...
function HelperPersonnelFrame:drawSolidRect(x, y, width, height, r, g, b, a)
    if self.lineOverlay == nil then
        return
    end
...
function HelperPersonnelFrame:drawPersonPortrait(person, x, y, width, height, selected)
    ...
    if selected then
        self:drawSolidRect(x - 0.003, y - 0.005, width + 0.006, 0.0022, 0.61, 0.73, 0.07, 1)
        self:drawSolidRect(x - 0.003, y + height + 0.003, width + 0.006, 0.0022, 0.61, 0.73, 0.07, 1)
        self:drawSolidRect(x - 0.003, y - 0.005, 0.0015, height + 0.010, 0.61, 0.73, 0.07, 1)
        self:drawSolidRect(x + width + 0.002, y - 0.005, 0.0015, height + 0.010, 0.61, 0.73, 0.07, 1)
    end
end

function HelperPersonnelFrame:drawPersonMiniRow(person, x, y, width, isApplicantView, entryIndex)
    local manager = self:getManager()
    if manager == nil then
        return
    end
...
function HelperPersonnelFrame:getBaseWorkerHiredLine(manager, person)
...
function HelperPersonnelFrame:getWorkerMarkerLine(manager, person)
...
function HelperPersonnelFrame:getWorkerWageAndStatusLines(manager, person)
...
function HelperPersonnelFrame:getWorkerSalaryRequestLine(manager, person)
...
function HelperPersonnelFrame:drawPersonCard(person, entryIndex, selectedIndex, x, y, width, height, isApplicantView)
    local manager = self:getManager()
    ...
    local detailTextSize = HelperPersonnelFrame.DETAIL_TEXT_SIZE or 0.0106
    local lineStep = HelperPersonnelFrame.DETAIL_TEXT_LINE_STEP or 0.014
...
function HelperPersonnelFrame:getVisibleEntryRange(selectedIndex, count, isApplicantView)
    local visibleCount = math.min(count, HelperPersonnelFrame.DETAIL_VISIBLE_COUNT or 4)
...
function HelperPersonnelFrame:drawDetailScrollbar(x, y, width, height, firstIndex, visibleCount, count)
...
function HelperPersonnelFrame:drawTextLine(x, y, size, align, text, r, g, b, a, bold)
...
function HelperPersonnelFrame:getTextSegmentWidth(size, text, bold)
    text = tostring(text or "")
...
function HelperPersonnelFrame:drawTextSegment(x, y, size, text, r, g, b, a, bold)
    text = tostring(text or "")
...
function HelperPersonnelFrame:getPrimaryDetailLabelWidth(size, text)
    text = tostring(text or "")
...
function HelperPersonnelFrame:getDetailValueColumnOffset(size, lines)
    ...
    return maxLabelWidth + (HelperPersonnelFrame.DETAIL_LABEL_VALUE_GAP or 0.008)
end

function HelperPersonnelFrame:drawDetailTextLine(x, y, size, text, r, g, b, a, valueColumnOffset)
    text = tostring(text or "")
...
function HelperPersonnelFrame:historyLineFits(text, textSize, maxWidth, fallbackMaxChars)
    text = tostring(text or "")

    if maxWidth ~= nil and getTextWidth ~= nil then
        local measuredSize = getCorrectTextSize ~= nil and getCorrectTextSize(textSize) or textSize
        return getTextWidth(measuredSize, text) <= maxWidth
    end

    return string.len(text) <= (fallbackMaxChars or 42)
end

function HelperPersonnelFrame:getWrappedHistoryLines(text, textSize, maxWidth, fallbackMaxChars)
...
function HelperPersonnelFrame:getOverviewHistoryHeader(manager, period, year)
    local label = nil
...
function HelperPersonnelFrame:buildOverviewHistoryRows(lines, textSize, maxWidth)
    local rows = {}
...
function HelperPersonnelFrame:drawMonthlyChangeHistory(manager, x, y, width, height)
    ...
    local lineHeight = HelperPersonnelFrame.OVERVIEW_HISTORY_LINE_HEIGHT or 0.0129
    ...
    HelperPersonnelFrame.OVERVIEW_HISTORY_VISIBLE_ROWS = visibleRows
...
function HelperPersonnelFrame:drawHistoryColumn(title, lines, x, y, maxWidth)
